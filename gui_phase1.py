...
for direction in DIRECTIONS:
    ...
    narrow_images[direction] = pygame.image.load(os.path.join(ARROW_PATH, f"{direction.lower()}_arrow.png"))
...
def phase():
    for i, direction in enumerate([d for d in DIRECTIONS for _ in range(PHASE_ONE_TRIALS_PER_DIRECTION)]):
        ...
        save_log(f"phase1_trial{i+1}_{direction}.csv")

def main():
    wait_for_space("Data Collection (1/2) - Press SPACE to begin")
    countdown()
    phase()
    wait_for_space("Phase 1 Complete - Press SPACE to exit")
    pygame.quit()
